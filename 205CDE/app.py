...
app.config['MYSQL_DB'] = 'stationary'
...
def search():
    ...
    if 'search' in request.args:
        ...
        query_string = "SELECT * FROM products WHERE ptname LIKE %s ORDER BY pid ASC"
...
#Def product type in Database
@app.route("/store" , methods=['GET', 'POST'])
def store():
    pdtypea = mysql.connection.cursor()
    values = 'kaweco'
    pdtypea.execute("SELECT * FROM products WHERE ptype=%s ORDER BY RAND() ", (values,))
    kaweco = pdtypea.fetchall()

    pdtypeb = mysql.connection.cursor()
    values = 'injector'
    pdtypeb.execute("SELECT * FROM products WHERE ptype=%s ORDER BY RAND() ", (values,))
    injector = pdtypeb.fetchall()

    pdtypeb = mysql.connection.cursor()
    values = 'ink'
    pdtypeb.execute("SELECT * FROM products WHERE ptype=%s ORDER BY RAND() ", (values,))
    ink = pdtypeb.fetchall()
    return render_template ('store.html', kaweco=kaweco, injector=injector, ink=ink)
...
def editpro():
    if 'pid' in request.args:
        ...
        if request.method == 'POST' and form.validate():
            ...
            # Create Cursor
            cur = mysql.connection.cursor()
            exe = cur.execute("UPDATE products SET ptname=%s, pdetail=%s, pprice=%s, ptype=%s , plink=%s WHERE pid=%s",
...
#upload new products from admin page
    ...
    if request.method == 'POST' and form.validate():
        ...
        if name and detail and price and ptype and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if savepic:
                    curs = mysql.connection.cursor()
                    curs.execute("INSERT INTO products( ptname, pdetail, pprice, ptype, plink)"
                                    "VALUES(%s, %s, %s, %s, %s)",
